...
const MenuBar = ()=>{
    ...
    const permisosPorRol = {
        ...
        3: ['Task Pending', 'Task In Progress', 'Task Complete', 'Profile', 'Log Out'],
    ...
    return(
        <div className={isOpen?'MenuContainerLateralOpen':'MenuContainerLateralClose'}>
            ...
            {canView('Dashboard Principal') && <button onClick={()=>{opcSelected(1,'/adminArea/principalTask')}} 
                ...
                {canView('Create Task') && <button onClick={()=>{opcSelected(2,'/adminArea/createTask')}} 
                ...
                {canView('Manage Area') && <button onClick={()=>{opcSelected(3,'/adminArea/principalTask')}} 
                ...
                {canView('Task Complete') && <button onClick={()=>{opcSelected(7,'/adminArea/principalTask')}} 
                        className={`btn_menu ${isOpen ? "btn_menu_ha_opc_open" : "btn_menu_ha_opc_close"} ${Selected == 7 ? "Selected" : ""}`}>
                    <i className="fa-regular fa-user fa-xl Icons_menu_bar"></i>
                    {isOpen?<label>Task Complete</label>:''}
                </button>}
                {canView('Profile') && <button onClick={()=>{opcSelected(8,'/adminArea/principalTask')}} 
